...
const DataStore = {
  ...
  // Generate time slots for calendar views (2-hour blocks for schedule display)
  getTimeSlots() {
    return [
  ...
  // Get jadwal for a specific day and time slot
  getJadwalAtSlot(day, jamMulai, jamSelesai) {
    return this.jadwal.filter(j =>
      j.hari === day && j.jam_mulai === jamMulai && j.jam_selesai === jamSelesai
