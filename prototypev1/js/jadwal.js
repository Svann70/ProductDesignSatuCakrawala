...
const Jadwal = {
  ...
  viewKalender(filtered) {
    ...
    filtered.forEach(j => {
      ...
      if (gridData[slotKey] && gridData[slotKey][j.hari]) {
        gridData[slotKey][j.hari].push(j);
      }
    });
    ...
    return `
      <div style="margin-bottom: var(--space-3); display: flex; gap: var(--space-4); flex-wrap: wrap; align-items: center">
        ...
        <div style="display: flex; gap: var(--space-3); flex-wrap: wrap; margin-left: auto">
          ${DataStore.jurusan.slice(0, 5).map(j => `
          ...
          `).join('')}
          <span style="font-size: var(--text-xs); color: var(--color-ink-subdued)">...</span>
        </div>
      </div>
          ...
          ${DataStore.hari.map(hari => {
            ...
            return `
              <div class="calendar-cell" data-hari="${hari}" data-jam="${slot.start}" onclick="Jadwal.onCellClick('${hari}', '${slot.start}', '${slot.end}')">
                ${items.map(j => {
                  const color = DataStore.getJurusanColor(j.jurusan_id);
                  return `
  ...
  openForm(presetHari, presetJamMulai, presetJamSelesai) {
        ...
        <div class="modal-body">
          ...
          <div class="form-group">
            ...
            <select class="form-select" id="formDosen">
              ...
              ${DataStore.dosen.filter(d => d.status === 'Aktif').map(d => `<option value="${d.id}" ${jadwal?.dosen_id === d.id ? 'selected' : ''}>${d.nama}</option>`).join('')}
            </select>
          </div>
          ...
          <div class="form-group" style="margin-top: var(--space-4)">
            ...
            <select class="form-select" id="formRuangan">
              ...
              ${DataStore.ruangan.filter(r => r.is_active).map(r => {
                const g = DataStore.getGedung(r.gedung_id);
                return `<option value="${r.id}" ${jadwal?.ruangan_id === r.id ? 'selected' : ''}>${r.nama} - ${g?.nama || ''} (${r.kapasitas} kursi)</option>`;
              }).join('')}
            </select>
    ...
    // Show dosen preference
    const dosenSelect = document.getElementById('formDosen');
    dosenSelect.addEventListener('change', () => this.showPreference());
    if (jadwal?.dosen_id) this.showPreference();

    // Validate on change
    ['formHari', 'formJamMulai', 'formJamSelesai', 'formRuangan', 'formDosen'].forEach(id => {
